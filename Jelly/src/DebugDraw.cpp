...
void DrawRect(const int32 &vertexCount, const b2Vec2 * vertices, const b2Color & color)
{
    ...
    for (int i = 0; i < vertexCount; i++)
    {
        ...
        //Hazel::Renderer2D::DrawQuad({ v.x*RATIO, v.y*RATIO }, { PNTSIZE, PNTSIZE }, { color.r, color.g, color.b, color.a * ALPHA });
    }

    auto p = glm::vec2(sum.x / vertexCount, sum.y / vertexCount);
    Hazel::Renderer2D::DrawQuad({ p.x*RATIO, p.y*RATIO, PZ }, { w*RATIO, h*RATIO }, { color.r, color.g, color.b, color.a * ALPHA });
}
...
void DrawLine(const b2Vec2 & p1, const b2Vec2 & p2, const b2Color & color)
{
    ...
    //    Hazel::Renderer2D::DrawQuad({ v.x * RATIO, v.y * RATIO, PZ + 0.2f }, { PNTSIZE, PNTSIZE }, { color.r, color.g, color.b, color.a * ALPHA });
    //}
    DebugDraw::DrawLine(a* RATIO, b* RATIO, { color.r, color.g, color.b, color.a  * ALPHA });
}
...
void DebugDraw::DrawCircle(const b2Vec2& center, float radius, const b2Color& color)
{
    ...
    //if (m_BeginDraw != nullptr) m_BeginDraw();
    //Hazel::Renderer2D::DrawQuad({ center.x*RATIO, center.y*RATIO, PZ }, { radius * .5f*RATIO, radius* .5f *RATIO }, { color.r, color.g, color.b, color.a* ALPHA });
    //if (m_EndDraw != nullptr) m_EndDraw();
}
...
void DebugDraw::DrawSolidCircle(const b2Vec2& center, float radius, const b2Vec2& axis, const b2Color& color)
{
    if (m_BeginDraw != nullptr) m_BeginDraw();
    Hazel::Renderer2D::DrawQuad({ center.x*RATIO, center.y*RATIO, PZ }, { radius* .5f*RATIO, radius* .5f *RATIO }, { color.r, color.g, color.b, color.a* ALPHA });
    ::DrawLine(center, { center.x + radius * axis.x, center.y + radius * axis.y }, color);
    if (m_EndDraw != nullptr) m_EndDraw();
}
...
void DebugDraw::DrawPoint(const b2Vec2& p, float size, const b2Color& color)
{
    if (size > 1.f)
        size = 1.f;
    if (m_BeginDraw != nullptr) m_BeginDraw();
    Hazel::Renderer2D::DrawQuad({ p.x*RATIO, p.y*RATIO, PZ + 0.1f }, { size*RATIO, size *RATIO }, { color.r, color.g, color.b, color.a* ALPHA });
    if (m_EndDraw != nullptr) m_EndDraw();
}
